...
- **If the coder prompt says a library is available, the runtime must guarantee it**: The coder agent was told "Available libraries: PIL (Pillow), imageio, requests" — so it wrote code using them. But those packages weren't installed in the worker container, causing `exec()` to fail with `No module named 'PIL'`. Any library listed as available in the coder prompt must be verified (and installed if missing) before `exec()` is called.
