...
- **Recovery agents must be general, not narrow**: A hardcoded coder prompt with GIF-specific instructions only fixes GIF. The recovery agent needs the failure context, the existing tool catalog, and autonomy to figure out what to implement — then use `shell_exec` to install packages, test code, and iterate. Hardcoding any specific technology in the recovery path defeats the purpose of self-healing.
- **Dynamic import scanning beats a fixed package list**: Instead of a hardcoded list of packages to pre-install, scan the generated code's import statements and install only what's actually missing. This handles any library the coder chose to use, not just the ones predicted upfront.
...
- **[config] QDRANT_URL is the canonical Qdrant env var — never `QDRANT_HOST`**
  - **Trigger:** Any new code, compose file, shell script, `.env*` template, or doc that needs to configure a Qdrant endpoint.
  - **Why:** Having two overlapping env vars (`QDRANT_URL` vs `QDRANT_HOST`) caused a silent ingestion bug: `HybridMemoryStore` only honors `QDRANT_URL`, so setting `QDRANT_HOST` left `self.qdrant=None` and every upsert was a no-op — the CLI cheerfully reported `added: 11` with nothing landing in Qdrant. A single canonical variable eliminates this class of bug. `QDRANT_URL` wins because it already dominates the codebase (observability, memory, worker, shared store all read it), it's strictly more expressive than host alone (scheme + host + port in one value), and it matches the `redis://`, `postgres://`, `temporal://` convention used elsewhere.
  - **Rule:** ALWAYS use `QDRANT_URL` in the form `http://host:port`. NEVER introduce or reintroduce `QDRANT_HOST`, `QDRANT_PORT`, or `QDRANT_HOST_PORT`.
  - **Enforcement:** `tests/test_qdrant_env_normalization.py` greps every tracked file and fails if `QDRANT_HOST` reappears. The test itself and this lessons entry are the only allowlisted mentions.
  - **Example:** `.env` → `QDRANT_URL="http://localhost:6333"`. `docker-compose.*.yml` → `QDRANT_URL=${QDRANT_URL:-http://qdrant:6333}`. `src/config.py` parses the URL into `config["qdrant"]["host"]`/`["port"]` for legacy consumers, so you only ever set the URL.
