        ...
        if self.queue_url == "dummy-temporal-queue":
            ...
            try:
                                        ...
                                        try:
                                            ...
                                            if phase == "recovery_analyzing":
                                                ...
                                                if BUDGET_TRACKING_ENABLED and budget_tracker:
                                                    try:
                                                        budget_status = await budget_tracker.get_status(hb.get("provider", "openrouter"))
                                                        provider_status = budget_status.get(hb.get("provider", "openrouter"), {})
                                                        spent = provider_status.get("spent", 0)
                                                        limit = provider_status.get("limit", 0)
                                                        if limit > 0:
                                                            pct = (spent / limit) * 100
                                                            display += f" | Budget: ${spent:.2f}/${limit:.0f} ({pct:.0f}%)"
                                                    except Exception:
                                                        pass
                ...
                try:
                    import redis
                    redis_cfg = self.config.get("redis", {})
                    r = redis.Redis(host=redis_cfg.get("host", "localhost"), port=redis_cfg.get("port", 6379), decode_responses=True)
                    last_sum = r.get("budget:last_summary")
                    if last_sum:
                        budget_summary = f"\n💰 Budget: {last_sum}"
                    r.close()
                except Exception:
                ...
                if result.get('mode') == 'agent':
                    ...
                    self._send_text(
                        ...
                        telegram=(
                            ...
                            f"{artifact_note}{budget_summary}\n\n"
                            f"📋 *Summary:*\n{truncated}"
                        ),
                    )
                else:
                    ...
                    self._send_text(
                        ...
                        plain=(
                            ...
                            f"Cost: ${cost:.6f} USD{budget_summary}\n\n"
                            f"Result:\n{brief}"
                        ),
                        telegram=(
                            f"✅ *Task Succeeded*\n"
