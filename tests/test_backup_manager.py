...
from src.genesis.orchestrator.backup_manager import BackupManager


@pytest.fixture
def mock_backup_manager(tmpdir):
    with patch("src.genesis.orchestrator.backup_manager.load_settings") as mock_load_settings:
...
@patch("src.genesis.orchestrator.backup_manager.requests.get")
@patch("src.genesis.orchestrator.backup_manager.requests.post")
...
@patch("src.genesis.orchestrator.backup_manager.subprocess.run")
...
@patch("src.genesis.orchestrator.backup_manager.BackupManager.backup_qdrant", return_value=True)
@patch("src.genesis.orchestrator.backup_manager.BackupManager.backup_temporal", return_value=True)
